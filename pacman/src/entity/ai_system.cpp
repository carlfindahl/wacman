...
void AISystem::recieve_pacmanstate(const EvPacInvulnreableChange& pac)
{
    if (pac.is_invulnerable)
    {
        ...
        m_reg.view<CAI>().each([](uint32_t e, CAI& ai) { ai.state = EAIState::Scared; });
    }
    else
    {
        GFX_DEBUG("Ghosts no longer scared -> Searching")
        m_reg.view<CAI>().each([](uint32_t e, CAI& ai) { ai.state = EAIState::Searching; });
    }
}
...
void AISystem::pathfind(const glm::ivec2& ai_pos, const glm::ivec2& ai_dir, const glm::ivec2& plr_pos, CAI& ai)
{
    ...
    /* Scattering means going to random locations (sort of) */
    case EAIState::Scattering: ai.target = m_level.find_closest_intersection(ai_pos, ai_dir); break;
    ...
    case EAIState::Scared: ai.target = m_level.find_sensible_escape_point(ai_pos, ai_dir, plr_pos); break;
