...
void RenderingSystem::update(float dt, entt::registry& reg)
{
    ...
    regular.each([](auto e, const CSprite& sprite, const CPosition& pos) {
        get_renderer().draw({glm::vec2(pos.position) * TILE_SIZE<float>, glm::vec2(TILE_SIZE<float>, TILE_SIZE<float>),
    ...
    regular_moving.each([](auto e, const CSprite& sprite, const CPosition& pos, const CMovement& move) {
        ...
        get_renderer().draw(
            {interp_pos * TILE_SIZE<float>, glm::vec2(TILE_SIZE<float>, TILE_SIZE<float>), glm::vec3(1.f), sprite.sprite});
    });
    ...
    regular_moving_anim.each([](auto e, const CAnimationSprite& sprite, const CPosition& pos, const CMovement& move) {
        ...
        get_renderer().draw({interp_pos * TILE_SIZE<float>, glm::vec2(TILE_SIZE<float>, TILE_SIZE<float>), glm::vec3(1.f),
                             sprite.active_animation});
    });
}
}  // namespace pac
