...
bool ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    // Store GLSL version string so we can refer to it later in case we recreate shaders. Note: GLSL version
    // is NOT the same as GL version. Leave this to NULL if unsure.
#ifdef USE_GL_ES3
...
#ifdef GL_SAMPLER_BINDING
    glBindSampler(
        0, 0);  // We use combined texture/sampler state. Applications using GL 3.3 may set that otherwise.
#endif
...
bool ImGui_ImplOpenGL3_CreateFontsTexture()
{
    ...
    io.Fonts->GetTexDataAsRGBA32(
        &pixels, &width,
        &height);  // Load as RGBA 32-bits (75% of the memory is wasted, but default font is so small) because
...
// If you get an error please report on github. You may try different GL context version or GLSL version. See
// GL<>GLSL version table at the top of this file.
...
bool ImGui_ImplOpenGL3_CreateDeviceObjects()
{
    ...
    if (glsl_version == 410 || glsl_version == 430)
    {
