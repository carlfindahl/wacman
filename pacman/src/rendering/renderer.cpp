...
namespace pac
{
Renderer::Renderer(unsigned max_sprites) : m_vao(std::vector<VertexArray::Attribute>{{{0u, 0u, 2, GL_FLOAT, offsetof(Vertex, pos), 0u},
                                                                                      ...
                                                                                      {5u, 1u, 1, GL_UNSIGNED_INT, offsetof(InstanceVertex, texture_id), 1u}}})
{
    ...
    prog = std::make_unique<ShaderProgram>(std::vector<cgl::ShaderStage>{{GL_VERTEX_SHADER, "res/sprite.vert"},
                                                                         {GL_FRAGMENT_SHADER, "res/sprite.frag"}});
    ...
    glNamedBufferStorage(m_element_buffer, cgl::size_bytes(sprite_indices), sprite_indices.data(), GL_MAP_READ_BIT);
    glNamedBufferStorage(m_sprite_buffer, cgl::size_bytes(sprite_quads), sprite_quads.data(), GL_MAP_READ_BIT);
    ...
    glVertexArrayElementBuffer(m_vao, m_element_buffer);
    glVertexArrayVertexBuffer(m_vao, 0u, m_sprite_buffer, 0u, sizeof(Vertex));
